...
class MachineCom(object):
  ...
  def _monitor(self):
    ...
    while self._monitoring_active:
      try:
        ...
        if "ok C:" in line or "Count" in line:
          self._update_pos(line)
  ...
  def _update_pos(self, l):
    bits = m114exp.findall(l)
    x = None
    y = None
    z = None
    z1 = None
    ...
    self.Zpos.SetLabel(z1)'''
    # MainWindow.savezpos(self,z1)
  ...
  def _onConnected(self):
    ...
    self._currentZ = settings().getFloat(["serial", "zpos"])
    if self._currentZ >=0:
      self.sendCommand("G92 Z%.4f " % self._currentZ)
      self.sendCommand("M114")
      self._callback.on_comm_z_change(self._currentZ)
