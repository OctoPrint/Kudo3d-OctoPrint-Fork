$(function() {
    function Terminal1ViewModel(parameters) {
        var self = this;
        ...
        self.WiFiSetup = function() {
                    $("#validate_dialog1 .validate_dialog1_message").text(gettext("Please wait for a minute and refresh the browser to switch to the other wireless mode!"));
                    $("#validate_dialog1").modal("show");
                $.ajax({
        ...
        self.WiFiReset = function() {
                    $("#validate_dialog1 .validate_dialog1_message").text(gettext("Please wait for a minute and refresh the browser to switch to the other wireless mode!"));
                    $("#validate_dialog1").modal("show");
                $.ajax({
                    url: API_BASEURL + "wifireset",
                    type: "POST",
                    dataType: "json",
                    data: "action="+self.ssid()+" "+self.password(),
                    success: function() {
                        //new PNotify({title: "Success", text: _.sprintf(gettext("The command \"%(command)s\" executed successfully"), {command: "WiFi Reset"}), type: "success"});
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
                        var error = "<p>" + _.sprintf(gettext("The command \"%(command)s\" could not be executed."), {command: "WiFi Set"}) + "</p>";
                        error += pnotifyAdditionalInfo("<pre>" + jqXHR.responseText + "</pre>");
